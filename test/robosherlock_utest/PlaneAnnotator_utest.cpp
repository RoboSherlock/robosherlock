...
int planeTest()
{
  ...
  if(plane_coefficients->values.empty())
	{ 
  	...
  	{
           if (plane_inliers->indices.size() > 10000 ) number_of_points = true;
           else number_of_points = false;
  	}
  engine.getAnnotatorContext().releaseCAS(*cas);
  engine.collectionProcessComplete();
}
TEST(UnitTest, CheckExistingPlane)
{
   planeTest();
   EXPECT_EQ(1,exist_plane);
}
TEST(UnitTest, CheckPlaneNumberOfPoints)
{
   EXPECT_EQ(1,number_of_points);
}
