...
private:
    ...
    // MPS
    std::vector<pcl::PlanarRegion<pcl::PointXYZRGBA>, Eigen::aligned_allocator<pcl::PlanarRegion<pcl::PointXYZRGBA>>>
    regions;
...
public:
    PlaneAnnotator()
            ...
            , mode(BOARD)
            , display(new pcl::PointCloud<pcl::PointXYZRGBA>())
            , saveToFile(false)
            , pointSize(1)
    {
    ...
    TyErrorId initialize(AnnotatorContext& ctx)
    {
        ...
        if (ctx.isParameterDefined("plane_direction"))
        {
            ctx.extractValue("plane_direction", planeDirection);
        }
...
private:
    TyErrorId processWithLock(CAS& tcas, ResultSpecification const& res_spec)
    {
        ...
        if (!planes.empty())
        {
            loadPlaneModel(tcas, scene, true);
            foundPlane = true;
        }
        else
        {
    ...
    void estimateFromBoard(CAS& tcas, rs::Scene& scene)
    {
        ...
        cv::solvePnPRansac(pointsWorld, pointsImage, cameraMatrix, distortionCoefficients, rotation, translation,
                           !rotation.empty() && !translation.empty(), 100, 1.0);
    ...
    void estimateFromMPS(CAS& tcas, rs::Scene& scene)
    {
        ...
        for (size_t i = 0; i < regions.size(); ++i)
        {
            ...
            plane_inliers->indices.insert(plane_inliers->indices.end(), inlierIndices[i].indices.begin(),
                                          inlierIndices[i].indices.end());
        }
    ...
    void loadPlaneModel(CAS& tcas, rs::Scene& scene, bool from_annotation = false)
    {
        ...
        if (!from_annotation)
        {
            ...
            {
                float num = fabs(planeModel[0] * cloudFiltered->points[i].x + planeModel[1] * cloudFiltered->points[i].y +
                                 planeModel[2] * cloudFiltered->points[i].z + planeModel[3]);
                float denum = std::sqrt(std::pow(planeModel[0], 2) + std::pow(planeModel[1], 2) + std::pow(planeModel[2], 2));
        ...
        else
        {
            ...
            if (!planes.empty())
            {
                planeModel[0] = planes[0].model.get()[0];
                planeModel[1] = planes[0].model.get()[1];
                planeModel[2] = planes[0].model.get()[2];
                planeModel[3] = planes[0].model.get()[3];
                ...
                planes[0].inliers(plane_inliers->indices);
                planes[0].roi(rs::conversion::to(tcas, roi));
                planes[0].mask(rs::conversion::to(tcas, mask));
    ...
    bool process_cloud(pcl::ModelCoefficients::Ptr& plane_coefficients)
    {
        ...
        if (planeDirection == "vertical" || planeDirection == "horizontal") {
            Eigen::Vector3f axis;
            ...
            plane_segmentation.setEpsAngle(55.0f * (M_PI/180.0f));
