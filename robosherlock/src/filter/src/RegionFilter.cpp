...
private:
    TyErrorId processWithLock(CAS& tcas, ResultSpecification const& res_spec)
    {
        ...
        for (size_t i = 0; i < semantic_map_items_.size(); ++i)
        {
            ...
            obj.name(item.name);
            obj.typeName(item.reference_frame);
        ...
        for (auto region : regions_to_look_at_)
        {
            ...
            item.name = it->name;
            item.reference_frame = it -> reference_frame;
            item.y_dimension = it -> y_dimension;
            item.x_dimension = it -> x_dimension;
            item.z_dimension = it -> z_dimension;
            item.type = it -> type;
            item.plane_eq = it -> plane_eq;
            if (it != semantic_map_items_.end())
            {
                outInfo("Found " << region);
                outInfo(it->name << " is defined in " << it->reference_frame);
                if (item.reference_frame != "map" && rs::TFListenerProxy::listener->frameExists(item.reference_frame)) {
                    outInfo("Looking up transfrom from: " << camera_info_.header.frame_id << " to "
                                                          << item.reference_frame);
                    listener_.listener->waitForTransform(camera_info_.header.frame_id, item.reference_frame,
                                                         ros::Time(0),
                                                         ros::Duration(2.0));
                    ...
                    listener_.listener->lookupTransform("map", item.reference_frame, ros::Time(0),
                                                        regionToWorld);
                    item.transform = regionToWorld * it -> transform;

                    computeFrustum();

                } else {
                    item.transform = it -> transform;
                }

                if (frustumCulling(item) || !frustum_culling_)
                {
                    outInfo("region inside frustum: " << item.name);
                    filterRegion(item);

                    if (item.has_plane_equations)
                    {
                        outInfo("has plane equations");
                        ...
                        Eigen::Vector4d plane_eq(item.plane_eq[0], item.plane_eq[1], item.plane_eq[2], -item.plane_eq[3]);
                ...
                outWarn("Region: " << region << " is not defined in the semantic map that was loaded.");
        }
    ...
    void filterRegion(const SemanticMapItem& region)
    {
        ...
        if (publish_marker_) {
            publishSemanticMapMarker(region);
        }
    }
