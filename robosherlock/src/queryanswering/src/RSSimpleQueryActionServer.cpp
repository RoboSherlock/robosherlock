...
RSSimpleQueryActionServer::RSSimpleQueryActionServer(ros::NodeHandle n, RSProcessManager* rsMngr){
            this->run(n, rsMngr);
 ...
 * Server
 */
RSSimpleQueryActionServer::~RSSimpleQueryActionServer(){
...
void RSSimpleQueryActionServer::sendFeedback(std::basic_string<char> status){
     robosherlock_msgs::RSSimpleQueryFeedback feedback;
...
std::string* RSSimpleQueryActionServer::objectDesignator_to_jsonString(const robosherlock_msgs::RSSimpleQueryGoalConstPtr objPtr){
...
robosherlock_msgs::ObjectDesignator* RSSimpleQueryActionServer::jsonString_to_objectDesignator(std::string jsonStr){
                                                    ...
                                                    if(answer["rs.annotation.PoseAnnotation"][i].HasMember("world")){
                                                        if(answer["rs.annotation.PoseAnnotation"][i]["world"].HasMember("rs.tf.StampedPose")){
                                                            ...
                                                            if(answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"].HasMember("timestamp"))
                                                                objPtr->pose[i].header.seq=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["timestamp"].GetInt64();
                                                            if(answer.HasMember("timestamp"))
                                                             ...
                                                             if(answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"].HasMember("frame"))
                                                                 objPtr->pose[i].header.frame_id=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["frame"].GetString();

                                                             //poseStamped's orientation
                                                             if(answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"].HasMember("rotation")){
                                                                 objPtr->pose[i].pose.orientation.x=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[0].GetDouble();
                                                                 objPtr->pose[i].pose.orientation.y=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[1].GetDouble();
                                                                 objPtr->pose[i].pose.orientation.z=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[2].GetDouble();
                                                                 objPtr->pose[i].pose.orientation.w=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[3].GetDouble();
                                                             }

                                                             //poseStamped's position
                                                             if(answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"].HasMember("translation")){
                                                                 objPtr->pose[i].pose.position.x=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["translation"].GetArray()[0].GetDouble();
                                                                 objPtr->pose[i].pose.position.y=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["translation"].GetArray()[1].GetDouble();
                                                                 objPtr->pose[i].pose.position.z=answer["rs.annotation.PoseAnnotation"][i]["world"]["rs.tf.StampedPose"]["translation"].GetArray()[2].GetDouble();
                                                             }
                                                        }
                                                    }
                                                ...
                                                if(answer["rs.annotation.PoseAnnotation"].HasMember("world")){
                                                    if(answer["rs.annotation.PoseAnnotation"]["world"].HasMember("rs.tf.StampedPose")){
                                                        ...
                                                        if(answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"].HasMember("timestamp"))
                                                            objPtr->pose[0].header.seq=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["timestamp"].GetInt64();
                                                        if(answer.HasMember("timestamp"))
                                                         ...
                                                         if(answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"].HasMember("frame"))
                                                             objPtr->pose[0].header.frame_id=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["frame"].GetString();

                                                         //poseStamped's orientation
                                                         if(answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"].HasMember("rotation")){
                                                             objPtr->pose[0].pose.orientation.x=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[0].GetDouble();
                                                             objPtr->pose[0].pose.orientation.y=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[1].GetDouble();
                                                             objPtr->pose[0].pose.orientation.z=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[2].GetDouble();
                                                             objPtr->pose[0].pose.orientation.w=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["rotation"].GetArray()[3].GetDouble();
                                                         }

                                                         //poseStamped's position
                                                         if(answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"].HasMember("translation")){
                                                             objPtr->pose[0].pose.position.x=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["translation"].GetArray()[0].GetDouble();
                                                             objPtr->pose[0].pose.position.y=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["translation"].GetArray()[1].GetDouble();
                                                             objPtr->pose[0].pose.position.z=answer["rs.annotation.PoseAnnotation"]["world"]["rs.tf.StampedPose"]["translation"].GetArray()[2].GetDouble();
                                                         }
                                                    }
                                                }
  ...
  */
 void RSSimpleQueryActionServer::executeQuery(const robosherlock_msgs::RSSimpleQueryGoalConstPtr& goal,RSSimpleQueryActionServer* rSSimpleQueryActionServer,RSProcessManager* rsMngr)
 {

    //set ProcessManager
     rSSimpleQueryActionServer->rsProcessManager=rsMngr;
    //shared result pointer
    robosherlock_msgs::RSSimpleQueryResult result;
    //publish a feedback about query activation
    std::basic_string<char > status="Query received: 0% completion";
    rSSimpleQueryActionServer->sendFeedback(status);

   status="Starting query processing ...";
   rSSimpleQueryActionServer->sendFeedback(status);

   std::string goalString=*rSSimpleQueryActionServer->objectDesignator_to_jsonString(goal);
   ...
   if(rsMngr!=nullptr){
       if(!(rsMngr->handleQuery(goalString, resString))){
           ...
           status="Error: Aborted Query - Check query formatting!!!";
            rSSimpleQueryActionServer->sendFeedback(status);
            rSSimpleQueryActionServer->server->setAborted(result);
       }else{
           //query answer and status
           status="Result Processing: 75% completion";
           rSSimpleQueryActionServer->sendFeedback(status);
           for(int i=0;i<resString.size();i++){
               result.res.push_back(*(rSSimpleQueryActionServer->jsonString_to_objectDesignator(resString[i])));
           }
           status="Succeded Query: 100% completion";
           rSSimpleQueryActionServer->sendFeedback(status);
           rSSimpleQueryActionServer->server->setSucceeded(result);
 ...
 int RSSimpleQueryActionServer::run(ros::NodeHandle n, RSProcessManager* rsMngr){

     ROS_INFO("Server was started!");
     this->server=new Server(n, std::string("/rs_query_action_").append(getenv("USER")), boost::bind(&(RSSimpleQueryActionServer::executeQuery), _1,this,rsMngr), false);
     this->server->start();
     return 0;
 }
 ...
 int main(int argc, char** argv)
 {
     ...
     RSSimpleQueryActionServer actionServer(n,rsMngr);
     ros::spin();
     return 0;
 }
