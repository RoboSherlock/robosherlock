...
bool RSProcessManager::handleQuery(std::string &request, std::vector<std::string> &result)
{
  ...
  queryType = queryInterface->processQuery(newPipelineOrder[0]);
  {
    ...
    if(queryType == QueryInterface::QueryType::DETECT) {
      ...
      visualizer_.setActiveAnnotators(newPipelineOrder[0]);

      engine_.setNextPipeline(newPipelineOrder[0]);
      engine_.applyNextPipeline();
      ...
      if(!newPipelineOrder[0].empty()) {
        outInfo("planned new pipeline: ");
        for(auto s : newPipelineOrder[0])
          outInfo(s);
        inspectionEngine_.setNextPipeline(newPipelineOrder[0]);
      ...
      engine_.setNextPipeline(newPipelineOrder[1]);
      engine_.applyNextPipeline();
      engine_.process(resultDesignators, request);
      engine_.resetPipelineOrdering();
      return true;
