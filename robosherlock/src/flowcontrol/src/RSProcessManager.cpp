  ...
  visualizer_(savePath, !useVisualizer)
{
  ...
  query_interface = new QueryInterface(knowledgeEngine_);
}
    ...
    robosherlock_msgs::ExecutePipeline::Response &res)
{
  ...
  std::vector<std::string> new_pipeline_orders[0] = req.annotators;
  outInfo("Setting new pipeline: ");
  for(auto a : new_pipeline_orders[0])
  {
  ...
  if(use_identity_resolution_ && std::find(new_pipeline_orders[0].begin(), new_pipeline_orders[0].end(), "ObjectIdentityResolution") == new_pipeline_orders[0].end())
    new_pipeline_orders[0].push_back("ObjectIdentityResolution");
...
bool RSProcessManager::handleQuery(std::string &request, std::vector<std::string> &result)
{
  ...
  if (!new_pipeline_orders.empty())
  {
    ...
    {
      rapidjson::Document &query_document = query_interface->getQueryDocument();
