...
RSAggregateAnalysisEngine::RSAggregateAnalysisEngine(uima::AnnotatorContext& rANC, bool bOwnsANC,
                                                     bool bOwnsTAESpecififer, uima::internal::CASDefinition& casDefs,
                                                     bool ownsCasDefs)
...
uima::TyErrorId RSAggregateAnalysisEngine::annotatorProcess(std::string annotatorName, uima::CAS& cas,
...
uima::TyErrorId RSAggregateAnalysisEngine::annotatorProcess(int index, uima::CAS& cas,
                                                            uima::ResultSpecification& resultSpec)
{
  if (index < 0 || index >= static_cast<int>(iv_annotatorMgr.iv_vecEntries.size()))
  {
    outError("Provided index is invalid in current annotator flow. Index: " << index << ", flow size: "
                                                                            << iv_annotatorMgr.iv_vecEntries.size());
...
uima::TyErrorId RSAggregateAnalysisEngine::parallelProcess(uima::CAS& cas, uima::ResultSpecification const& resSpec)
{
  ...
  for (size_t order = 0; order < currentOrderingIndices.size(); order++)
  {
    ...
    for (size_t i = 0; i < primitiveProcessStatus.size(); i++)
    {
      ...
      {
        outError("Exception throws at annotator: " << currentOrderings[order][i]
                                                   << ". Pipeline will stop at this iteration!");
        return err;
      }
    }
  }
...
bool RSAggregateAnalysisEngine::planParallelPipelineOrderings(
    std::vector<std::string>& annotators, RSParallelPipelinePlanner::AnnotatorOrderings& orderings,
...
void RSAggregateAnalysisEngine::processOnce(std::vector<std::string>& designator_response, std::string queryString)
{
  ...
  try
  {
    icu::UnicodeString ustrInputText = name_.c_str();
    cas_->setDocumentText(uima::UnicodeStringRef(ustrInputText));
    cas_->setIdentifier(name_);
    rs::StopWatch clock;
...
bool RSAggregateAnalysisEngine::initParallelPipelineManager()
{
  try
  {
    ...
    querySuccess =
        this->planParallelPipelineOrderings(currentFlow, this->currentOrderings, this->currentOrderingIndices);
...
void RSAggregateAnalysisEngine::setPipelineOrdering(std::vector<std::string> annotators)
{
  ...
  for (int i = 0; i < annotators.size(); i++)
  {
    ...
    outWarn(annotators.at(i) << " can't be found in your loaded AnalysisEngine. Maybe it has not been "
...
std::string convertAnnotatorYamlToXML(std::string annotator_name,
                                      std::map<std::string, rs::AnnotatorCapabilities>& delegate_capabilities)
{
    ...
    try
    {
      ...
      else
      {
        if (delegate_capabilities[annotator_name].oTypeValueDomains.empty())
          delegate_capabilities[annotator_name].oTypeValueDomains =
              converter.getAnnotatorCapabilities().oTypeValueDomains;
...
// the following three methods are a workaround for the very limited constructors that uima::internal::AggregateEngine
// offers
RSAggregateAnalysisEngine* createRSAggregateAnalysisEngine(const std::string& ae_file, bool parallel)
{
  ...
  RSAggregateAnalysisEngine* engine = nullptr;

  engine =
      (RSAggregateAnalysisEngine*)(rs::createParallelAnalysisEngine(AEXMLFile.c_str(), delegateMapping, errorInfo));
...
RSAggregateAnalysisEngine* createParallelAnalysisEngine(
    icu::UnicodeString const& aeFile, const std::unordered_map<std::string, std::string>& delegateEngines,
    uima::ErrorInfo errInfo)
{
  try
  {
    ...
    std::auto_ptr<uima::internal::CASDefinition> apCASDef(
        uima::internal::CASDefinition::createCASDefinition(*apANC.get()));
    // release auto_ptrs here because the createTAE transfers ownership to the engine
    apTAESpecifier.release();
    RSAggregateAnalysisEngine* pResult =
        rs::createParallelAnalysisEngine(*apANC.release(), *apCASDef.release(), errInfo);
    return pResult;
  }
                                                        ...
                                                        uima::ErrorInfo& errInfo)
{
  ...
  try
  {
    ...
    try
    {
      utErrorID = pResult->initialize(crTAESpecifier);
    }
    catch (rs::InitAEException& ex)
    {
      outError(ex.what());
    }
